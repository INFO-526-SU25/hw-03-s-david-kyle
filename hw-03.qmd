...
```{r}
# Step 1: Pivot longer to get all responses in one column
...
# Step 2: Count responses per question and score
...
# define label mapping and desired order
question_labels <- c(
  resp_safety            = "Based on my\nunderstanding, I\nbelieve the\nvaccine is safe",
...
# apply label mapping, factor order
likert_summary <- likert_summary |>
  mutate(
    question = recode(question, !!!question_labels),
...
# plot
ggplot(likert_summary, aes(x = percent_signed, y = question, fill = score)) +
  geom_col(width = 0.8) +
  scale_fill_manual(
    values = likert_colors,
    name = "Response",
    breaks = c("1", "2", "3", "4", "5") # Force the legend order to be numerical here
  ) +
  scale_x_continuous(labels = abs, name = "Percentage of Responses") +
  labs(
    title = "COVID-19 Vaccine Attitudes by Question",
    y = NULL
  ) +
  theme_minimal() +
  theme(
...
likert_summary_hundo <- likert_summary |>
  mutate(
    question = recode(question, !!!question_labels),
    question = factor(question, levels = question_order),
    score = factor(score, levels = score_levels), # score factor levels updated here for plotting order
...
# plot
div_plot <- ggplot(likert_summary_div, aes(x = percent_signed, y = question, fill = score)) +
  geom_col(width = 0.8) +
  scale_fill_manual(
    values = likert_colors,
    name = "Response",
...
hundo_plot <- ggplot(likert_summary_hundo, aes(x = percent_signed, y = question, fill = score)) +
  geom_col(width = 0.8) +
  scale_fill_manual(
    values = likert_colors,
    name = "Response",
